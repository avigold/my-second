...
def api_player_repertoire(slug: str, color: str):
    ...
    queue = _deque([(start_board.copy(), root_node, 0)])
    total_nodes = 1

    while queue and total_nodes < MAX_NODES:
        board, parent_node, depth = queue.popleft()
        if depth >= MAX_DEPTH:
            continue
        ...
        for i, m in enumerate(top_moves):
            ...
            total_nodes += 1
            queue.append((after, child_node, depth + 1))
...
def api_admin_players_delete(slug: str):
    ...
    if not player:
        return jsonify({"error": "not found"}), 404
    # Remove book JSON files if they exist.
...
def api_admin_players_upload_photo(slug: str):
    ...
    # Remove any existing photo for this slug (may have different extension)
